...
- **Devices merged into the Participants admin page** — a new Devices column (a participant can have multiple sessions) and a "Revoke Access" action that revokes all of a participant's sessions at once, sourced from the existing `GET`/`DELETE /admin/sessions*` endpoints. The standalone Devices page is unchanged and still needed: it's the only place that surfaces sessions with no matching participant record, which the merged Participants view has no row to attach them to.
