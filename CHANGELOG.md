...
- **Deliver questionnaires on habit creation** (SRHI by default): a questionnaire assignment can now be flagged **"Deliver first occurrence on habit creation (counts as week 1)"** in Studies → Schedule (`deliverOnHabitCreation`). When a participant creates a habit (`POST /habits/intentions`), the backend generates that questionnaire's week-1 window **per habit** anchored at creation time and fires a fire-and-forget FCM push (~5 s later, reusing the existing device-token → FCM path, deep-link `{type: srhi, intentionId}`) instead of anchoring the first window at enrollment. SRHI is special-cased so the flag drives its own per-habit pipeline (`srhiService.generateWindows` → 4 weekly `srhi_responses`, week 1 = today), preserving its scoring/sparkline; other flagged questionnaires create one `questionnaire_windows` row per habit (`questionnaire_windows` now carries `intentionId`, and its unique key includes it). The **default study seeds an SRHI assignment with the flag on**, idempotently and back-filling any pre-existing default study (`ensureSrhiHabitCreationAssignment`). New service functions `generateHabitCreationWindows` / `srhiDeliversOnHabitCreation`; admin Schedule form gains the checkbox + a badge. See `DOCUMENTATION.md` §8 → Questionnaire Scheduling & Check-in Delivery and `docs/data-model.md`.
