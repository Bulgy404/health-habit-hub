...
- **Optional self-hosted product analytics on a separate private VM.** Added a
  configurable, fail-closed PostHog deployment package around an immutable
  upstream revision, with separate Kafka/ZooKeeper, ClickHouse, PostgreSQL,
  Redis/Valkey, object-storage, Temporal, application and ingestion containers;
  private-only binds; resource limits; host/container exporters; PostgreSQL and
  ClickHouse backups; checksum manifests; optional offsite copy; and a systemd
  timer. The existing Traefik publishes only a rate-limited `/ingest` endpoint
  allowlist when configured, while Prometheus/Grafana cover private PostHog
  reachability, both hosts' disk/memory/container pressure and an early ingest
  budget warning. Flutter and Node use one versioned strict event registry with
  autocapture/replay disabled; onboarding, habit creation, recommendation and
  enrollment funnels are instrumented without free text. Recommendation IDs
  now persist through adopted implementation intentions and drive admin
  acceptance statistics. Empty environment values keep the entire feature
  disabled until the future VM exists.
...
- **Notifications (both FCM pushes and local habit/SRHI reminders) never showed while the app was open on iOS.** `firebase_messaging` registers its own `UNUserNotificationCenter` delegate, which by default suppresses the system alert/sound/badge presentation for _any_ notification while the app is foregrounded — including ones shown locally via `flutter_local_notifications`, unrelated to FCM. Fixed by calling `FirebaseMessaging.setForegroundNotificationPresentationOptions(alert/badge/sound: true)`.
