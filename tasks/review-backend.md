...
7. **Extract business logic from route files into service modules.**
   ...
   **Resolution (US-156, OF-12 ✅ Resolved):** Business logic extracted from `adminRouter.js` into three service modules: `app/services/adminParticipantService.js` (`listParticipants`, `createParticipant`, `assignGroup`, `getParticipant`, `softDeleteParticipant`), `app/services/adminHabitService.js` (`getHabitsFeed`, `buildHabitsCSV`), and `app/services/adminStatsService.js` (`getParticipantProgress`, `getSettings`, `updateSetting`). Each service accepts `{ neo4jRun, db }` (plus `kc` where Keycloak is needed). `adminRouter.js` reduced to route definitions and thin controller calls — no inline Cypher strings or business logic remain. Unit tests added in `app/tests/unit/admin.services.test.js` covering all three services with mocked `neo4jRun`.
