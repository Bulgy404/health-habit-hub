...
> **Colors:** always read brand colors through `context.appColors.*` / `AppColors` (`lib/theme/app_colors.dart`) — never a hardcoded hex. In particular, don't reach for `primary` (`#45B700`) on a solid-fill button; that shade fails WCAG AA contrast for white text. See [`docs/design-system.md`](../design-system.md) for the full token table and the primary-vs-primaryDark rule.
...
`widgets/contribution_graph_widget.dart` is a GitHub-style calendar heatmap (weeks as columns, month/weekday labels, colour intensity by count) used in three places:
...
- **`/donate` (share screen)** — a separate, on-device-only counter (`SharedPreferences`, not backend logs) of how many times the participant has used "share a habit" per day.
...
**Known pitfall — build the grid's day-stepping loop in UTC, not local time.** The grid is generated by walking from the oldest visible week to today one day at a time (`cursor = cursor.add(const Duration(days: 1))`, plus a single large `rangeStart = today.subtract(Duration(days: daysBack))`). `DateTime.add`/`.subtract` on a **local** `DateTime` is real-elapsed-time based (exactly 24h/duration), not calendar-day based. Since the graph typically spans 20+ weeks, that walk almost always crosses at least one EU Daylight Saving Time transition (last Sunday of March / last Sunday of October) — a 23-hour or 25-hour calendar day. Stepping "+1 day" in local time across a 23-hour day overshoots to 01:00 instead of 00:00, which silently drops a calendar day from the grid and shifts every date after it one weekday out of alignment (confirmed live: a habit logged on a real Monday rendered in the grid's Sunday row, for every date after the March 2026 transition). The fix is to do all the day-stepping in UTC (`DateTime.utc(...)`, immune to DST) and convert back to a local `DateTime(y, m, d)` only once each day is stored, for lookups against `today` and `widget.counts`. If you touch this widget's grid-construction logic again, keep the arithmetic in UTC — don't reintroduce `.add`/`.subtract(Duration(days: ...))` on a local `DateTime`.
