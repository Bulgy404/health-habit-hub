...
![Critical alerts flow: backup runs and LLM-unavailable events fire directly from application code via alerting.py/send_smtp_mail(); BullMQ job failures, service-down, and 5xx-spike events are metric-driven through Grafana's unified alerting; both paths send to the same ALERT_EMAIL using the same SMTP credentials](assets/architecture/critical-alerts-flow.svg)
