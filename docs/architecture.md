...
> **Related:** the full diagram suite (system architecture, UML use case diagram, 41 per-use-case sequence diagrams, domain class diagram) lives in [`docs/diagrams/`](diagrams/README.md) as renderable Mermaid/PlantUML sources. The use case catalogue with code traceability is in [`diagrams/use-cases/use-case-overview.md`](diagrams/use-cases/use-case-overview.md).
...
## Product Analytics and Cross-VM Monitoring

Product analytics is optional and fail-closed. The tracked configuration can be
deployed with every PostHog variable blank; in that state the Flutter and Node
clients are no-ops, Traefik renders no `/ingest` router, and Prometheus loads no
remote analytics targets.

When configured, PostHog runs on a dedicated TU-internal VM using the official
hobby Compose stack at an immutable Git revision plus the reviewed override in
`analytics-vm/`. PostgreSQL, Redis/Valkey, ClickHouse, Kafka/ZooKeeper, object
storage, Temporal, web, worker, capture and ingestion remain separate
containers. The override removes upstream public dependency ports, binds the
single PostHog HTTP port and both exporters to the VM's private address, applies
memory limits, and pins mutable PostHog images by digest.

There are two event paths:

1. Flutter sends only registry-approved events to
   `https://${DOMAIN}/ingest`. Traefik rate-limits the route, strips `/ingest`,
   and forwards only exact PostHog ingestion endpoint families over the private
   network. It never forwards `/`, so the PostHog UI is not public.
2. The Node backend sends authoritative outcomes such as recommendation
   latency/failure and recommendation adoption directly to
   `POSTHOG_SERVER_HOST` on the private network.

Both clients consume `app/analytics/event-registry.json`. The Flutter registry
is generated and checked in CI. Unknown events, extra keys, free-form values and
wrong types are rejected. Every accepted event adds `study_id`, `group_id`,
`app_version`, `platform`, `locale`, and `schema_version`; pre-enrollment values
use the controlled `not_assigned` sentinel. Session replay, lifecycle
autocapture, surveys, feature-flag events, push autocapture and exception steps
are disabled in the Flutter SDK. PostHog's native `$identify`/`$set` protocol
record is the sole non-registry event: the wrapper supplies only the opaque
Keycloak subject plus validated `study_id` and `group_id`, never profile text or
PII.

Traefik does not expand environment values inside file-provider YAML. The
one-shot `traefik-config` container therefore runs
`monitoring/render-traefik.sh`, validates the domain/private origin/rate limits,
and atomically writes ordinary YAML to the `hhh-traefik-dynamic` volume before
the proxy starts. With a blank `POSTHOG_INTERNAL_URL` it writes only
`http: {}`. Renderer tests cover disabled, enabled and malicious/invalid input.

Prometheus on `habitvm` scrapes local and remote node-exporter/cAdvisor targets
and blackbox-probes PostHog's private health endpoint. `ANALYTICS_VM_HOST` is
rendered into file-discovery targets by a one-shot container; a blank value
produces empty target arrays. Grafana alerts on exporter/reachability failures,
low disk or host memory, and containers approaching their memory limits.
Traefik exposes internal Prometheus metrics on `proxy:8082`; the PostHog router
label feeds a conservative 30-day ingest-budget warning without inspecting
event bodies or granting monitoring an administrative PostHog credential.

Researchers open PostHog only from the TU network/VPN. Administrators use
`analytics-vm/manage.sh` for host preflight, pinned-source preparation, Compose
validation, startup, logs, status and database backups. See
`docs/analytics-posthog-plan.md`, `analytics-vm/README.md`, and UC-40/UC-41.
...
Two production-only traps, both of which surface as `error=OAuthCallback` _after_ Keycloak has already authenticated the user:
...
- Every other page is admin-only — `researcher` users are redirected to `/access-denied`: `/participants`, `/donations`, `/comments`, `/profile-fields`, `/knowledge-base`, `/system`, `/backups`, `/audit-log`, `/restore-attempts`, `/team`
- `/participants` shows each participant's registered devices inline (a Devices column — one participant can have several) sourced from `GET /admin/devices`; clicking it opens a modal with each device's platform, model, app version, and last-seen time — `platform`/`model`/`appVersion` come from the mobile app itself (see `POST /participant/register-token` below), not from Keycloak, which has no concept of device metadata. Separately, a "Revoke Access" action force-signs-out the participant everywhere by revoking every one of their Keycloak sessions (`GET`/`DELETE /admin/sessions*`) — this is deliberately independent of the Devices column: a "device" (a `deviceTokens` registration) and a "session" (a Keycloak login) aren't reliably the same thing, so the two are tracked, displayed, and fetched separately even though both live on this one page. There is no longer a standalone Devices page — it was removed once device data moved here; the old page could only ever show Keycloak session metadata (no model/app version existed anywhere), which is what this change replaces.
- `/donations` rows are clickable, opening a detail view (transcript, inline audio playback + download, self-report answers, linked post-donation questionnaire response) backed by the `/admin/habit-donations/:uuid[/audio]` routes above.
- The sidebar groups navigation into five labelled sections — **Research** (Studies, Analytics), **Operations** (Participants, Donations, Comments), **Configuration** (Cue Pools, Questionnaires, Profile Fields, Knowledge Base), **Monitoring** (System, Backups, Audit Log, Restore Attempts, Team), and **Support** (Help) — and hides every admin-only entry for `researcher` users so the navigation reflects what they can actually open
...
2. **enrol** — HHH does exactly what it does for an anonymous code. Group
   allocation stays in HHH: the register knows _who_ someone is, not which arm
   they belong in.
...
| **Filesystem volume**       | Docker named volume (`audio-recordings-data`, mounted read-write at `AUDIO_STORAGE_DIR` in the `app` container and read-only in the `backup` service) | Recorded habit-donation voice clips, named `{uuid}.{ext}`, referenced by `habit_donations.audioClip`                                                                                                                                                     | Small binary blobs unrelated to Mongo's document workload; only written once a donation actually completes. Included in the scheduled backup/restore pipeline (tar-based, same pattern as the LightRAG volume) and a non-critical `audioStorage` write-probe in `GET /health`, since the volume being unwritable by the non-root `app` container user on a fresh mount is a real failure mode this stack has hit in production. |
...
| `/admin/habit-donations/:uuid`                  | Donation detail: transcript, linked `form_responses` doc, and (accepted habits only) the Neo4j `Habit` node's self-report properties                                                                                                                                                                                                        |
| `/admin/habit-donations/:uuid/audio`            | Streams the recorded audio clip (`?download=1` for attachment; inline playback otherwise)                                                                                                                                                                                                                                                   |
| `POST /participant/register-token`              | Registers/refreshes an FCM push token plus (optionally) `deviceId`/`platform`/`model`/`appVersion` — a `deviceId` keeps this device's `deviceTokens` doc separate from the participant's other devices instead of overwriting them; omitting it (older app builds) falls back to one doc per participant, as before device tracking existed |
| `/admin/devices`                                | Every registered device across all participants (paginated), cross-referenced to `username`/`participantStatus` the same way `/admin/sessions` is — powers the Participants page's Devices column/modal                                                                                                                                     |
...
to be seeded (as _definitions_ — this does not pre-assign or activate either
