...
- `/participants` also shows each participant's active device sessions inline (a Devices column — one participant can have several) and a "Revoke Access" action that revokes all of them at once, sourced from the same `GET`/`DELETE /admin/sessions*` endpoints the standalone `/devices` page uses. `/devices` itself still exists separately: it's the only place that surfaces sessions with no matching participant record (`participantStatus: "no_matching_participant"`), which `/participants` has no row to attach them to.
