...
test('Mongo connect + CRUD (integration)', async (t) => {
  ...
  if (!ping.ok) {
    ...
    if (
      ping.error &&
      (ping.error.includes('Authentication failed') ||
        ping.error.includes('ECONNREFUSED'))
    ) {
      return t.skip(
        `MongoDB not available at mongodb://${user}:***@${host}:${port}/ (db=${dbName}). Error: ${ping.error}`
      );
    }

    assert.fail(
      `Mongo not reachable at mongodb://${user}:***@${host}:${port}/ (db=${dbName}). Error: ${ping.error}`
    );
  }
  ...
  try {
    ...
    await db
      .collection('surveys')
      .insertOne({ id: surveyId, title: 'Test Survey', json: { pages: [] } });
