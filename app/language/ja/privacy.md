...
# アプリ「Health Habit Hub」における個人データの取り扱い

上記の各項目はウェブサイトに関するものです。以下では、研究の一環として提供される
モバイルアプリ「Health Habit Hub」における個人データの取り扱いについて、
補足的に説明します。

## 管理者および連絡先

本研究はドレスデン工科大学（Technische Universität Dresden）経営情報学講座
（特に Health Informatics）が実施しています。
本研究のデータ保護に関するお問い合わせ：digital-health@tu-dresden.de

## 参加の任意性

本研究への参加は任意です。同意はいつでも将来に向けて撤回でき、参加を終了できます。
撤回によって不利益が生じることはありません。

## 取り扱うデータ

本アプリはデータ最小化の原則に基づいて設計されています。**氏名、メールアドレス、
電話番号は取得しません。**

- **仮名アカウント：** 登録時にランダムな識別子を持つアカウントが作成されます。
  ログインは、お客様の端末上で生成される24単語のリカバリーフレーズによって
  行われます。これらのデータのみからお客様個人を特定することはできません。
- **お客様が入力する研究データ：** 提供された習慣（自由記述）、アンケートの回答
  および回答日時、任意のプロフィール情報（年齢層、性別など）。
- **研究への割り当て：** 参加している研究および研究グループ、参加日時。
- **プッシュ通知：** 通知を有効にした場合、リマインダーを配信するために端末識別子
  （プッシュトークン）が保存されます。
- **技術データ：** エラー分析および安全な運用のためのログデータ（アクセス日時、
  エラーメッセージなど）。
- **利用データ（プロダクト分析）：** アプリのどの部分が分かりやすく、どの部分で
  参加者が離脱するかを把握するため、あらかじめ定められた手順に到達したことを
  記録します。送信されるのは、プログラム内に定義された限定的な一覧に含まれる
  イベント（例：「onboarding_completed」「habit_created」）と、技術的な付随情報
  （アプリのバージョン、プラットフォーム、言語、研究およびグループの割り当て）
  のみです。これらのイベントはお客様の仮名識別子に紐づけられます。**お客様が
  入力したテキスト（習慣、目標、きっかけ、コメントなど）は送信されません。**
  画面内容の自動取得およびセッション記録は行いません。

## 目的および法的根拠

データは、健康習慣に関する学術研究の目的、アプリの改善および開発の目的、
ならびにアプリの技術的な運用のために処理されます。

法的根拠は、研究開始時にお客様が与える GDPR 第6条第1項(a)に基づく同意です。
入力内容が GDPR 第9条にいう健康データに該当する場合、処理は GDPR 第9条第2項(a)に
基づくお客様の明示的な同意に基づいて行われます。

## 提供先および処理者

- **ホスティング：** 研究データはドレスデン工科大学のサーバー上で処理されます。
- **言語モデルサービス：** 入力された習慣を自動的に分類するため、そのテキストが
  AIサービス（ScaDS.AI、llm.scads.ai）に送信されます。送信されるのは習慣の
  テキストのみであり、お客様の識別子は送信されません。
- **プッシュ通知：** 通知の配信には Firebase Cloud Messaging（Google Ireland
  Limited）を使用します。その際、プッシュトークンおよび通知内容が送信されます。
  通知は端末の設定からいつでも無効にできます。

- **プロダクト分析：** 利用データは、ドレスデン工科大学のサーバー上で自社運用する
  分析ソフトウェア PostHog により処理されます。商用の分析事業者に送信されることは
  ありません。

お客様のデータが広告目的で提供されることはありません。他社のアプリやウェブサイトを
横断したトラッキングも行いません。

## 保存期間

研究データは、研究期間中および良好な学術実践の規則に従ったその後の分析・保管期間に
わたって保存されます。

これに対し、「利用データ（プロダクト分析）」に記載した個々のイベントは、遅くとも
1年後に削除されます。そこから作成された、個人との関連を含まない集計結果は、
その後も保管される場合があります。

## お客様の権利

お客様には、アクセス、訂正、消去、処理の制限、データポータビリティ、および同意の
撤回を求める権利があります。アプリ内でいつでもご自身でデータをエクスポートできます。
また、データ保護監督機関に苦情を申し立てる権利も有しています。

## アカウントの削除

アプリの設定からいつでもご自身でアカウントを削除できます。削除により、アクセス権と
識別子が取り除かれ、ログインはできなくなり、これまでの投稿をお客様に結び付けることも
できなくなります。既に収集された研究への投稿は、**匿名化された**形でデータセットに
残ります。識別子の削除後は、いかなる個人にも結び付けることができないためです。これは
既に実施された学術的分析の完全性を保つためのものです。
