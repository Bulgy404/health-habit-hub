---
version: 1.2.0
effectiveDate: 2026-07-08
bindingLanguage: de
---

# Consentement à la participation à l'étude HabConnect

> **L'application Health Habit Hub** est l'application d'étude de l'étude **HabConnect – from Habit to Health**, menée par la Technische Universität Dresden.

> Ceci est une traduction de courtoisie. La version allemande du présent document fait foi.

---

## Points clés en un coup d'œil

Avant de lire tous les détails, voici un résumé rapide :

- **De quoi s'agit-il ?** Vous partagez vos habitudes quotidiennes et recevez en retour des recommandations personnalisées pour un mode de vie plus sain.
- **Qui mène l'étude ?** Le groupe de recherche Digital Health de la TU Dresden — examinée et approuvée par un comité d'éthique indépendant.
- **Mes données sont-elles en sécurité ?** Toutes les données sont stockées sous une forme **entièrement anonymisée**. Aucun nom, aucune adresse e-mail, aucune adresse postale.
- **La participation est-elle volontaire ?** Oui — vous pouvez arrêter à tout moment, pour quelque raison que ce soit, sans aucune conséquence.
- **Puis-je supprimer mes données ?** Comme toutes les données sont anonymes, il n'est pas possible d'identifier ni de supprimer des enregistrements individuels après l'anonymisation. Vous pouvez toutefois supprimer votre compte et l'ensemble des données associées dans les paramètres de l'application à tout moment, tant que vous participez encore.

---

## Informations

Chère participante, cher participant potentiel,

Merci de l'intérêt que vous portez à notre recherche. **L'application Health Habit Hub** fait partie de l'étude **HabConnect – from Habit to Health**, planifiée et menée par le groupe de recherche Digital Health, Faculté des sciences économiques et de gestion, Technische Universität Dresden. L'étude est autofinancée par notre établissement et a été examinée par un comité d'éthique indépendant, qui n'a soulevé aucune objection.

Votre participation est volontaire. Vous pouvez arrêter à tout moment sans avoir à en donner la raison et sans que cela n'entraîne d'inconvénient pour vous.

## Pourquoi cette étude est-elle menée ?

Le projet de recherche comporte deux volets :

**Volet 1 – Collecte des habitudes :** Nous recueillons des données sur les habitudes quotidiennes — quelles habitudes les personnes ont, dans quels contextes, et en quoi elles diffèrent selon les groupes.

**Volet 2 – Test des recommandations :** Sur la base des données collectées, nous développons et testons la manière dont les recommandations personnalisées doivent être formulées et transmises pour que les personnes les adoptent réellement.

Exemples de questions de recherche :

- Dans quels contextes (par exemple, moment de la journée, environnement) les habitudes favorables à la santé sont-elles le plus souvent réalisées ?
- Quels facteurs influencent le fait que les personnes intègrent de nouvelles habitudes dans leur quotidien ?
- Comment les interventions numériques influencent-elles le maintien à long terme d'habitudes saines ?

L'objectif est de développer des recommandations scientifiquement fondées et applicables au quotidien, qui aident les personnes à mener une vie plus saine sur le long terme.

## Comment se déroule l'étude ?

Il n'y a pas de durée de participation fixe — vous décidez vous-même combien de temps et avec quelle intensité vous participez.

**Dans l'application Health Habit Hub, vous pouvez :**

- Décrire et partager vos habitudes quotidiennes (par exemple : « Tous les matins, après le réveil, je me brosse les dents et fais 10 squats. »)
- Recevoir des recommandations personnalisées pour des habitudes saines et indiquer si vous les avez suivies
- Remplir de courts questionnaires hebdomadaires (environ 10 minutes)

Selon votre degré de participation, le temps quotidien nécessaire est d'environ 5 à 15 minutes.

## Qu'y gagnez-vous ?

- Vous réfléchissez plus consciemment à vos propres habitudes.
- Vous recevez des idées et des recommandations personnalisées pour un quotidien plus sain.
- Vous apportez une contribution précieuse à la recherche, qui peut bénéficier à de nombreuses personnes sur le long terme.

## Quels sont les risques ?

Hormis les aspects liés à la protection des données expliqués ci-dessous, la participation ne comporte aucun risque.

## Qu'attend-on de vous ?

Veuillez répondre à toutes les questions honnêtement et au mieux de votre connaissance. Ce n'est qu'ainsi que des résultats fiables peuvent être obtenus et des recommandations pertinentes peuvent en être tirées.

## Protection des données

Le groupe de recherche Digital Health de la TU Dresden est responsable de vos données.

**Base juridique :** Votre consentement personnel (art. 6 (1) a), art. 9 (2) a) du RGPD).

### Quelles données sont collectées ?

Toutes les données sont collectées sous une forme **entièrement anonymisée**. Cela signifie qu'aucune information permettant de vous identifier n'est enregistrée — ni nom, ni date de naissance, ni adresse e-mail.

Nous collectons :

- Des descriptions d'habitudes quotidiennes
- Des caractéristiques démographiques (par exemple, tranche d'âge, sexe, profession) — uniquement sous forme anonymisée
- Le comportement d'utilisation de l'application (par exemple, si une habitude recommandée a été suivie)
- Les réponses aux questionnaires hebdomadaires sur la force des habitudes et le bien-être

### Comment les données sont-elles utilisées ?

Vos données anonymisées seront utilisées pour :

1. **Cette étude** — afin de répondre aux questions de recherche
2. **Une base de données ouverte sur les habitudes** — les données anonymisées seront mises durablement à la disposition de la communauté scientifique afin de développer de meilleurs systèmes de recommandation
3. **Des publications scientifiques** — sous une forme ne permettant aucune conclusion sur des participants individuels

### Transferts internationaux de données

Vos données peuvent être partagées avec des partenaires de recherche internationaux. Quel que soit le destinataire, seules des données anonymisées et agrégées sont transférées — jamais des enregistrements individuels identifiables.

Certains jeux de données anonymisés et agrégés (par exemple, sur la formulation des habitudes) sont partagés avec les partenaires suivants :

- **Tohoku University, Japon** (Center for Smart Aging Research)
- **Memorial University of Newfoundland, Canada** (Faculty of Business Administration)
- **Vrije Universiteit Brussel (VUB), Belgique**
- **Universitair Ziekenhuis Brussel (UZ Brussel), Belgique** (Department of Clinical Nutrition)

Le Japon et le Canada font tous deux l'objet d'une décision d'adéquation de l'UE. La Belgique est un État membre de l'UE ; le transfert de données vers ce pays n'est donc pas considéré comme un transfert international de données au sens du RGPD. Dans tous les cas, seules des données anonymisées sont transférées — par exemple : « Le groupe A a enregistré en moyenne plus d'habitudes que le groupe B. »

### Combien de temps les données sont-elles conservées ?

Les données anonymes sont stockées sur les serveurs de la TU Dresden et peuvent être conservées de manière permanente afin de soutenir la recherche scientifique à long terme. Une fois les données entièrement anonymisées, il n'est plus possible d'identifier des enregistrements individuels ni de les supprimer de manière sélective.

### Vos droits

Comme toutes les données sont anonymes, nous ne pouvons attribuer aucun enregistrement spécifique à une personne après l'anonymisation. Pour toute question ou préoccupation, veuillez contacter :

**Felix Reinsch**
felix.reinsch@tu-dresden.de
Groupe de recherche Digital Health, Technische Universität Dresden

**Délégué à la protection des données de la TU Dresden :**
Helmholtzstr. 6, 01062 Dresden
Tél. : +49 351 463-0
Email : Informationssicherheit@tu-dresden.de

Vous avez le droit d'introduire une réclamation auprès de n'importe quelle autorité de contrôle en matière de protection des données :

**Déléguée saxonne à la protection des données et à la transparence**
Postfach 11 01 32, 01330 Dresden
Tél. : +49 351 85471-101
Email : post@sdtb.sachsen.de

Une liste de toutes les autorités de contrôle allemandes est disponible à l'adresse : https://www.bfdi.bund.de/DE/Infothek/Anschriften_Links/anschriften_links-node.html

## Mises à jour du présent formulaire de consentement

Ce formulaire de consentement est examiné régulièrement. Toute modification significative vous sera communiquée via l'application et un nouveau consentement vous sera demandé. La version actuelle est toujours accessible dans l'application.

## Déclaration de consentement

J'ai lu et compris les informations relatives à cette étude. J'ai été informé(e) que :

- ma participation est volontaire,
- je peux arrêter à tout moment sans avoir à en donner la raison,
- toutes mes données seront entièrement anonymisées,
- les données seront utilisées à des fins scientifiques et dans une base de données de recherche ouverte.

**En continuant, je consens à l'utilisation anonyme de mes données dans le cadre de la recherche HabConnect.**

Vos informations seront utilisées exclusivement à des fins scientifiques et stockées sur les serveurs sécurisés de la TU Dresden.
