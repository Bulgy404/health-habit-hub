...
export function createIntentionsRouter({ db, neo4jRun } = {}) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      // Recommendation ids are UUIDs generated by API-service/recommend.py.
      // Keep this aligned with recommendation_feedback.recommendation_id; an
      // ObjectId check here would reject every production recommendation.
      if (
        sourceRecommendationId != null &&
        (typeof sourceRecommendationId !== 'string' ||
          !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(
            sourceRecommendationId
          ))
      ) {
        return res.status(400).json({
          error: 'sourceRecommendationId must be a UUID v4 or null',
        });
      }
      if (
        reminderTime != null &&
      ...
      productAnalytics.capture({
        ...
        groupId,
        properties: {
          creation_mode: creationMode ?? 'standalone',
          from_recommendation: sourceRecommendationId != null,
        },
      });
      if (sourceRecommendationId != null) {
        productAnalytics.capture({
          distinctId: String(userId),
          event: 'recommendation_adopted',
          studyId,
          groupId,
          properties: { recommendation_id: sourceRecommendationId },
        });
      }
